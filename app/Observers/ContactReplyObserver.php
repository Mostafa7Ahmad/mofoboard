...
class ContactReplyObserver
{
    ...
    public function created(ContactReply $contactReply)
    {
        $email = null;
        $name = null;
        if ($contactReply->contact->email != null) {
            $email = $contactReply->contact->email;
        } elseif (isset($contactReply->contact->user->email)) {
            $email = $contactReply->contact->user->email;
        }

        if ($contactReply->contact->name != null) {
            $name = $contactReply->contact->name;
        } elseif (isset($contactReply->contact->user->name)) {
            $name = $contactReply->contact->user->name;
        }
