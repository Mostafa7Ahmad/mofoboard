                    ...
                    <table class="table table-bordered  table-hover">
                        ...
                        <tbody id="sortable-table">
                            @foreach ($comments as $comment)
                                <tr>
                                    <td class="ui-state-default drag-handler" data-comment="{{ $comment->id }}">
                                        {{ $comment->id }}</td>
                                    <td>
                                        <img class="rounded-circle mx-2" alt=""
                                            src="{{ $comment->user == null ? env('DEFAULT_IMAGE_AVATAR') : $comment->user->getUserAvatar() }}"
                                            style="width:20px;height: 20px;" />
                                        <a href="{{ $comment->user == null ? '#' : route('admin.users.show', $comment->user) }}"
                                            class="link-dark">{{ $comment->user == null ? mb_strimwidth($comment->adder_name, 0, 20, '...') : mb_strimwidth($comment->user->name, 0, 20, '...') }}</a>
                                    </td>
                                    <td>
                                        <div class="form-switch">
                                            <input name="id" class="form-check-input change_comment_status"
                                                type="checkbox" id="flexSwitchCheckDefault"
                                                {{ old('reviewed', $comment ?? 0) == '1' ? 'checked' : '' }}
                                    ...
                                    @if (auth()->user()->hasRole('superadmin'))
                                        <td><a
                                                href="{{ route('admin.traffics.logs', ['ip' => $comment->ip]) }}">{{ $comment->ip }}</a>
                                        </td>
                                    @endif
                                    <td>
                                        <a
                                            href="{{ route('article.show', ['article' => $comment->article, 'user' => $comment->article->user]) }}">{{ mb_strimwidth($comment->article->title, 0, 80, '...') }}</a>
                                    </td>
