                    ...
                    <tr>
                        ...
                        <th style="font-size: 12px">
                            @if ($report->user_id != null)
                                <a href="{{ route('admin.users.index') }}?id={{ $report->user_id }}">{{ $report->user_id }}
                                    - {{ $report->user->name }}</a>
                            @endif
                        </th>
                        ...
                        <th
                            style="font-size: 12px;background: {{ strpos($report->user_agent, 'bot') == false ? '#e6e6e6' : '#fff' }}">
                            <span class="fal fa-desktop" data-container="body" data-toggle="popover" data-placement="top"
                                data-content="{{ $report->user_agent }}"></span>
                        </th>
                        ...
                        <th style="font-size: 12px">{{ $report->created_at }} -
                            {{ \Carbon::parse($report->created_at)->diffForHumans() }}</th>
                        <th style="font-size: 12px">
