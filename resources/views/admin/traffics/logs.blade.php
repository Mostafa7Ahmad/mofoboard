        ...
        <div class="col-12 px-0 row d-flex pb-2" style="margin-top:15px">
            <form method="GET" action="{{ route('admin.traffics.logs') }}" class="row col-12 px-0">
                <div class="col-6 col-lg-2 pb-1">
                    <input type="" name="ip" placeholder="ip" value="{{ \Request::get('ip') }}"
                        class="form-control">
                </div>
                <div class="col-6 col-lg-2 pb-1">
            ...
            <tbody>
                @foreach ($logs as $log)
                    <tr>
                        <td style="background:@if ($log->user_id != null) green @endif">
                            @if ($log->user_id != null)
                                <a href="{{ route('admin.users.show', $log->user) }}"
                                    style="color:#fff;font-size:11px">{{ $log->id }}-{{ $log->user->name }}</a>
                            @else
                                {{ $log->id }}
                            @endif
                        </td>
                        ...
                        <td style="font-size: 12px;">
                            {{ $log->rate_limit->browser }} - {{ $log->rate_limit->device }} -
                            {{ $log->rate_limit->operating_system }} - {{ $log->rate_limit->country_name }}
                            <br>
                            <span c></span>
