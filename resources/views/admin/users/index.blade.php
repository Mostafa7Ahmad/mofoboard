            ...
            <div class="col-12 px-0">
                <div class="col-12 p-0 row">
                    <div class="col-12 col-lg-4 py-3 px-3">
                        <span class="fas mx-1fa-users"></span> المستخدمين
                    </div>
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    @if (auth()->user()->can('articles-read'))
                                        <td><a
                                                href="{{ route('admin.articles.index', ['user_id' => $user->id]) }}">{{ $user->articles_count }}</a>
                                        </td>
                                    @endif
                                    @if (auth()->user()->can('contacts-read'))
                                        <td><a
                                                href="{{ route('admin.contacts.index', ['user_id' => $user->id]) }}">{{ $user->contacts_count }}</a>
                                        </td>
                                    @endif
                                    @if (auth()->user()->can('comments-read'))
                                        <td><a
                                                href="{{ route('admin.article-comments.index', ['user_id' => $user->id]) }}">{{ $user->comments_count }}</a>
                                        </td>
                                    @endif
                                    @if (auth()->user()->can('traffics-read'))
                                        <td><a
                                    ...
                                    <td style="width: 1%;text-wrap: nowrap;">
                                        @include('components.control', [
                                            'links' => [
                                                ...
                                                [
                                                    'text' =>
                                                        "<div class='spinner-grow text-success' role='status' style='width:15px;height:15px'></div> مراقبة النشاط",
                                                    'icon' => 'fal fa-boxes',
                                                    'can' => 'users-update',
                                                    'url' => route('admin.traffics.logs', [
                                                        'user_id' => $user->id,
                                                    ]),
