@if ($general_popup_plugin = $website_plugins->where('activated')->where('slug', 'general_popup')->first())
    <div class="p-3 svg-animation general_popup_plugin_{{ data_get($general_popup_plugin->settings, 'popup_id', null) }}"
        style="position: fixed; left: 20px; bottom: -2000px; width: 450px; max-width: calc(-50px + 100vw); background: rgb(255, 255, 255); z-index: 10000; border-radius: 10px; box-shadow: rgba(10, 14, 29, 0.26) 0px 8px 13px 0px, rgba(119, 119, 119, 0.16) 0px 8px 64px 0px;transition: 0.5s all ease-in-out;">
        <style type="text/css">
        ...
        <div class="col-12 px-2 row d-flex align-items-center justify-content-center">
            <div class="col-10 px-0" style="color:#232323;font-size: 20px;font-weight: bold;">
                {{ $general_popup_plugin->settings['title'] ?? '' }}</div>
            <div class="col-2 d-flex align-items-center px-0 justify-content-end" onclick="close_general_popup_plugin()"
        ...
        @if ($general_popup_plugin->settings['btn_url'] ?? '' != '')
            <div class="col-12 p-2 d-flex align-items-end ">
                <a class="btn px-6 rounded col-12 general_popup_plugin_btn" onclick="close_general_popup_plugin()"
                    href="{{ $general_popup_plugin->settings['btn_url'] ?? '' }}" onclick="close_general_popup_plugin()"
                    @if (data_get($general_popup_plugin->settings, 'btn_url_in_new_tab', null)) target="_blank" @endif
