@if ($cross_pages_code = $website_plugins->where('slug', 'cross_pages_code')->first())
    @if (isset($cross_pages_code) &&
            data_get($cross_pages_code->settings, 'top_navbar_enable', false) &&
            ($top_navbar_content = data_get($cross_pages_code->settings, 'top_navbar_code', null)))
...
@if ($website_plugins->where('activated', 1)->where('slug', 'sticky_message')->count())
    ...
    @endphp
    @if (
        \Carbon::parse($sticky_message_plugin->settings['available_to'] ?? now())->gt(\Carbon::parse(now())) ||
            !isset($sticky_message_plugin->settings['available_to']))
        <div class="col-12 p-0 "
            style="background:{{ $sticky_message_plugin->settings['background_color'] ?? '#0194fe' }};overflow: hidden;">
            <div class="container p-0 {{ $sticky_message_plugin->settings['animate_type'] ?? '' }}"
                style="color:{{ $sticky_message_plugin->settings['text_color'] ?? '#FFFFFF' }};height:100%">


                @if (($sticky_message_plugin->settings['url'] ?? '') != '')
                    <a href="{{ $sticky_message_plugin->settings['url'] ?? '#' }}"
                        {{ $sticky_message_plugin->settings['url_in_new_tab'] ?? 0 == 1 ? 'target="_blank"' : '' }}
                        style="color:{{ $sticky_message_plugin->settings['text_color'] ?? '#FFFFFF' }};">
                @endif

                <div class="col-12 p-1 d-flex align-items-center row" style="height: 100%;">
                    <div class="col p-1 {{ ($sticky_message_plugin->settings['text_align_center'] ?? '') == 1 ? 'text-center' : '' }} "
...
<div class="col-12 fixed-top  main-nav shadow"
    style="background: #fff;padding: 3px 0px;min-height: 65px;position: sticky;top: 0px;">
            ...
            <div class="col me-auto p-0 row justify-content-between d-flex">
                        ...
                        @foreach ($menu->links as $link)
                            <div class="col-auto  d-none d-lg-flex align-items-center p-0 mx-1 ">
                                <a href="{{ $link->url }}"
                                    ...
                                    style="color: inherit;">
                                    {{-- <span class="{{$link->icon}} mx-1"></span> --}}
                                    {{ $link->title }}
                                </a>
                            </div>
                        @endforeach
                ...
                <div class="col-auto d-flex align-items-center px-1">
                        ...
                        <div class="btn-group" id="notificationDropdown">
                            ...
                            <div class="col-12 px-0 d-flex justify-content-center align-items-center "
                                ...
                                <span
                                    ...
                                    class="text-center" id="dropdown-notifications-icon">{{ $unreadNotifications }}</span>
                    ...
                    @if ($website_plugins->where('activated', 1)->where('slug', 'navbar_main_button')->count())
                        @php
                            $navbar_main_button = $website_plugins
                                ->where('activated', 1)
                                ->where('slug', 'navbar_main_button')
                                ->first();
                        @endphp

                        @if (auth()->check() && in_array($navbar_main_button->settings['visibility_type'] ?? '', ['ALL', 'AUTH']))
                            <a href="{{ $navbar_main_button->settings['btn_url_auth'] ?? '#' }}"
                                {{ $navbar_main_button->settings['url_in_new_tab_auth'] ?? 0 == 1 ? 'target="_blank"' : '' }}
                                class="{{ $navbar_main_button->settings['btn_class'] ?? '' }} {{ $navbar_main_button->settings['animate_type'] ?? '' }} py-1 px-3"
                        ...
                        @elseif(!auth()->check() && in_array($navbar_main_button->settings['visibility_type'] ?? '', ['ALL', 'GUEST']))
                            <a href="{{ $navbar_main_button->settings['btn_url_guest'] ?? '#' }}"
                                {{ $navbar_main_button->settings['url_in_new_tab_guest'] ?? 0 == 1 ? 'target="_blank"' : '' }}
                                class="{{ $navbar_main_button->settings['btn_class'] ?? '' }} {{ $navbar_main_button->settings['animate_type'] ?? '' }} py-1 px-3"
                                style="border-radius: 4px;">
    ...
    <div class="col-12 p-0 d-flex justify-content-between row flex-column" style="height: calc(100dvh - 67px);">
        <div class="col-12 p-0 aside-scroll pt-2" style="position: relative;overflow: auto;">
            ...
            @php
                $aside_menu = cache()->remember('menu_ASIDE_BAR', 60, function () {
                    return \App\Models\Menu::where('location', 'ASIDE_BAR')
                        ->with([
                            'links' => function ($q) {
                                $q->orderBy('order', 'ASC');
                            },
                ...
                <div class="col-12 p-0 row">
                    @foreach ($aside_menu->links as $link)
                        <div class="nav-item col-4 p-1">
                            <a href="{{ $link->url }}"
                                style="color: inherit;border-radius: 8px;border: 1px solid #f7f7f7;"
                                class="d-block p-1">
                                    ...
                                    <div class="col-12 px-0 row">
                                        <div class="col-12 px-0 kufi text-center font-1">
                                            ...
                                            @elseif($link->icon != null)
                                                <img src="{{ $link->icon() }}" style="max-width:30px;max-height:30px"
                                                    class="text-center">
                                            @else
                                                <span class="fal fa-cube font-4 mx-1"></span>
                                            @endif

                                            <div class="col-12 p-0 text-center pt-1 text-truncate"
        ...
        <div class="col-12 px-0 py-2" style="width: 100%;height: auto;">
            <div class="col-12 p-3 row">
                ...
                <div class="col p-1">
                    <a href="/login" style="border:2px solid #51c75b;color:inherit;"
                        class="col d-flex py-1 px-0 align-items-center justify-content-center btn">
                        دخول

                    </a>
                </div>
                <div class="col p-1">
                    <a href="/register" style="background:#51c75b;color:#fff;"
                        class="col d-flex py-1 px-0 align-items-center justify-content-center btn">
                ...
                <div class="col-12 p-0">
                    <ul style=";padding: 0px;list-style: none;"
                        ...
                        @if ($settings['facebook_link'] != null)
                            <a href="{{ $settings['facebook_link'] }}" class="d-inline-block p-1"
                                style="width:48px">
                        ...
                        @if ($settings['linkedin_link'] != null)
                            <a href="{{ $settings['linkedin_link'] }}" class="d-inline-block p-1"
                                style="width:48px">
                        ...
                        @if ($settings['telegram_link'] != null)
                            <a href="{{ $settings['telegram_link'] }}" class="d-inline-block p-1"
                                style="width:48px">
...
@if ($cross_pages_code = $website_plugins->where('slug', 'cross_pages_code')->first())
    @if (isset($cross_pages_code) &&
            data_get($cross_pages_code->settings, 'bottom_navbar_enable', false) &&
            ($bottom_navbar_content = data_get($cross_pages_code->settings, 'bottom_navbar_code', null)))
