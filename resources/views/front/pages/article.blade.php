            ...
            <div class="col-12 p-2 row d-flex">
                <div class="col-12 col-lg-8 p-2">
                    <div class="col-12 p-2 p-lg-3 rounded-2" style="background: var(--bg-main);">
                        <figure class="card-img-top"><img src="{{ $article->main_image() }}" alt="" data-fancybox />
                        </figure>
                            ...
                            <ul class="post-meta mb-5">
                                ...
                                @if ($article->comments_count != 0)
                                    <li class="post-comments"><a href="#comments"><i class="fal fa-comment"></i>
                                            {{ $article->comments_count }}<span> تعليقات</span></a></li>
                                @endif
                                @if ($article->views != 0)
                                    <li class="post-comments"><a href="#comments"><i class="fas mx-1fa-fa-thin fa-eyes"></i>
                                            {{ $article->views }}<span> مشاهدة</span></a></li>
                                @endif
                            </ul>
                            ...
                            @if (count($article->tags))
                                <div class="post-category text-line">
                                    @foreach ($article->tags as $article_tag)
                                        @if ($loop->index < 5)
                                            <a href="{{ route('tag.show', $article_tag) }}" class="hover pe-2"
                                                rel="tag">#{{ $article_tag->tag_name }}</a>
                                        @endif
                                    @endforeach
                                </div>
                            @endif
                                                ...
                                                <li class="comment">
                                                    <div class="comment-header d-md-flex align-items-center">
                                                        <div class="d-flex align-items-center">
                                                            <figure class="user-avatar ms-2 me-0"><img
                                                            ...
                                                            <div>
                                                                <h6 class="comment-author"><a href="#"
                                                                        class="link-dark font-1">{{ $comment->user == null ? $comment->adder_name : $comment->user->name }}</a>
                                                                </h6>
                                                                <ul class="post-meta">
                            ...
                            <div class="col-12 p-2">
                                <div class="col-12 p-0">
                                    <div class="col-12 aside-post "
                                        style="background: var(--second-background);border-radius: 5px;">
                                        @php
                                            $random_articles = \App\Models\Article::orderBy('id', 'DESC')
                                                ->with([
                                        ...
                                        @foreach ($random_articles as $single_article)
                                            <div class="col-12 p-0 d-flex row">
                                                <div style="width: 65px;padding: 3px;border-radius: 5px;overflow: hidden;">
                                                    <a href="{{ route('article.show', $single_article) }}"
                                                        class="px-0 d-flex">
                                                ...
                                                <div class="px-2" style="width:calc(100% - 75px)">
                                                    <div style="font-size:13px;font-weight: bold;" class="col-12">
                                                        <a href="{{ route('article.show', $single_article) }}"
                                                            class="px-0 d-flex" style="color:var(--font-color);">
                                                            {{ $single_article->title }} </a>
                                                    </div>
                                                    <div class="col-12 mt-1 font-small"
                                                        style="overflow: hidden;filter: grayscale(1);">
                                                        ...
                                                        <span class="posted-on"><span rel="bookmark"><span
                                                                    class="d-inline-block px-0 font-small"><time
                                                                        class="updated">{{ \Carbon::parse($single_article->created_at)->diffForHumans() }}</time><span></span></span>
                                                            </span></span>
                                                    </div>
                                                </div>
                                            </div>
